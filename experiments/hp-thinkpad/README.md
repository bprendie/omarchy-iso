...
`.dtbauto` sections and hardware-ID selection. The failed shared-power trial
is not packaged.
...
are deferred. See [the workbook](../../plans/2026-09-21-hp-t14-workbook.md),
[HP microphone findings](hp-mic-investigation.md), and
[T14 Bluetooth findings](t14-bluetooth-investigation.md).

The previous staged-root ISO repacking helpers have been retired. Historical
notes describe the tested images; the command above is the maintained build path.

## Validation of the clean-input workflow

Validated public downloads in a fresh native container against both archive and
final payload hashes. Built six hardware/support packages plus Dragon runtime
and settings in fresh ARM containers; the required ARM package-content checks
passed. Reproduced the tested base/patched T14 DTB hashes. The 52 selected unit
tests, BusyBox boot-hook test and both upstream helper-recipe tests pass.
No full ISO, Neovim source-package rebuild or physical boot of the new assembly
was performed during this workflow change. Those remain the next acceptance step.
