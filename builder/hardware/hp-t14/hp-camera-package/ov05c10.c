...
static int ov05c10_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl->id == V4L2_CID_VBLANK) {
		/* Control metadata must also be valid while the sensor is suspended. */
		vts = ov05c10->cur_mode->height + ctrl->val;
		...
		__v4l2_ctrl_modify_range(ov05c10->exposure,
			...
			min_t(s64, ov05c10->exposure->default_value, exposure_max));
	}
...
static int ov05c10_get_pm_resources(struct device *dev)
{
	...
	if (dev->of_node) {
		ov05c10->dovdd = devm_regulator_get(dev, "dovdd");
		if (IS_ERR(ov05c10->dovdd))
			return dev_err_probe(dev, PTR_ERR(ov05c10->dovdd), "dovdd unavailable\n");
		ov05c10->enable = devm_gpiod_get(dev, "enable", GPIOD_OUT_LOW);
		if (IS_ERR(ov05c10->enable))
			return dev_err_probe(dev, PTR_ERR(ov05c10->enable), "enable GPIO unavailable\n");
	}
	...
	ov05c10->reset = devm_gpiod_get_optional(dev, "reset", dev->of_node ? GPIOD_OUT_HIGH : GPIOD_OUT_LOW);
...
static int ov05c10_power_off(struct device *dev)
{
	...
	if (dev->of_node) {
		gpiod_set_value_cansleep(ov05c10->enable, 0);
		regulator_disable(ov05c10->dovdd);
	}
...
static int ov05c10_power_on(struct device *dev)
{
	...
	if (dev->of_node) {
		ret = regulator_enable(ov05c10->dovdd);
		if (ret) return ret;
		gpiod_set_value_cansleep(ov05c10->enable, 1);
...
static int ov05c10_probe(struct i2c_client *client)
{
	...
	full_power = dev->of_node || acpi_dev_state_d0(dev);
	if (full_power) {
...
static const struct of_device_id ov05c10_of_ids[] = {
	{ .compatible = "ovti,ov05c10" }, {}
};
MODULE_DEVICE_TABLE(of, ov05c10_of_ids);

static struct i2c_driver ov05c10_i2c_driver = {
	.driver = {
		.name  = "ov05c10",
		.of_match_table = ov05c10_of_ids,
