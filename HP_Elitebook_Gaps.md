...
References for reusable HP work: [camera implementation and physical results](../oma_snap/docs/hp-camera-status.md), [Fn/EC investigation](../oma_snap/docs/hp-unlock-fn-investigation.md), and [NPU acceptance test](../oma_snap/docs/hp-npu-validation.md). Camera and NPU success on the earlier build are useful baselines. Fn/backlight were not solved there, and a kernel swap cannot be assumed to solve them.
...
The [oma_snap HP NPU validation](../oma_snap/docs/hp-npu-validation.md) ran on the **same physical HP** with Ubuntu kernel `7.0.0-31-generic`: FastRPC calculator returned `499500` and `999`, the Qualcomm DSP validator passed, and the explicit HTP backend ID 6 ReLU graph returned `[0, 0, 0, 3]`. That separate runtime and its final harness are retained in the local oma_snap evidence/SDK archives, but the earlier result does not transfer to Dragon.
